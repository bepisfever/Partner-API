    ...
    if tracking then
        ...
        if radius >= minRadius then
            ...
            if prevAngle then
                ...
                if math.sign(totalAngle) ~= math.sign(diff) and totalAngle ~= 0 and diff ~= 0 then --Moving to another direction? Resets. (Works on higher refresh rate screens now, thanks AutumnMood!)
                    petTime = 0
                    totalAngle = 0
                end
